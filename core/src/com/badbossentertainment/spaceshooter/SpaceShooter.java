...
public class SpaceShooter extends ApplicationAdapter {
	...
	public void initializeVisualElements() {
		...
		ship = new Ship();
		//ship.setTextureRegion(texture_ship);
	...
	public void render () {
		...
		switch (GameModeManager.getMode()) {
			case Menu: {
				...
				ship.setX((Gdx.graphics.getWidth()-ship.getWidth())/2f);
				...
				shapeRenderer.circle(Gdx.graphics.getWidth() / 2f, Gdx.graphics.getHeight() / 2f, Gdx.graphics.getWidth() / 4f);
				...
				if (touch.isTouchUpRaised()) {
					...
					ship.setX(Gdx.graphics.getWidth() / 2f);
					ship.setY(Gdx.graphics.getWidth() / 8f);
			...
			case Died: {
				...
				shapeRenderer.circle(Gdx.graphics.getWidth() / 2, Gdx.graphics.getHeight() / 2, Gdx.graphics.getWidth() / 4);
			...
			// check if ship is dead
				...
				enemy.setX(MathUtils.random(0, Gdx.graphics.getWidth() - enemy.getWidth()));
				enemy.setY(Gdx.graphics.getHeight() + 1);
				enemies.add(enemy);
...
//			shapeRenderer.begin(ShapeRenderer.ShapeType.Filled); // begin shape renderer again
			...
			if (!screenTouched) {
				...
				shapeRenderer.getColor().a = 0.5f * (1-(gameSpeed-SLOW_MO_GAME_SPEED_LIMIT)/(1-SLOW_MO_GAME_SPEED_LIMIT));
