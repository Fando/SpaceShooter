...
//        }
    ...
    private void render(SpriteBatch batch) {
        if (region != null) {
            batch.begin();
            if (orientInDirectionOfVelocity) {
                batch.draw(region, position.x, position.y, size.x * 0.5f, size.y * 0.5f, size.x, size.y, 1, 1, dir);
            } else {
                batch.draw(region, position.x, position.y, size.x * 0.5f, size.y * 0.5f, size.x, size.y, 1, 1, orientation);
            }
            batch.end();
        }
        //sr.rect(position.x, position.y, size.x, size.y);
    ...
    private void updatePosition(float deltaTime) {
        // update position
        float scale = deltaTime / (Gdx.graphics.getDeltaTime()*1000);
